...
@pytest.fixture(params=[
    "6-methylflavone_Broken.mol2",
    "Apigenin_Broken.mol2", 
    "Chrysin_Broken.mol2",
    "Eriodictyol_Broken.mol2",
    "Fisetin_Broken.mol2"
])
def mol2_test_case(request, mol2_broken_files):
    """Parameterized fixture that returns (mol2_path, template_smiles) for each test case."""
    mol2_filename = request.param
    
    # Find the matching entry in mol2_broken_files
    for mol2_path, template_smiles in mol2_broken_files.items():
        if mol2_filename in mol2_path:
            return mol2_path, template_smiles
    
    pytest.fail(f"Could not find mol2 file {mol2_filename} in mol2_broken_files fixture")
...
class TestFixMol2:
    ...
    def test_hard_fix_mol2_function(
        ...
        mol2_path, template_smiles = mol2_test_case
